 ...
 */
@Service
public class OrderDeleteService extends AbstractB2S2CInfService<IdPo,ResultData> implements InterService<IdPo,ResultData> {
    private static final Logger logger = Logger.getLogger(OrderDeleteService.class);

    @Value("#{configProperties['api.b2s2c.order.delete.url']}")
    protected String            API_B2S2C_ORDER_DELETE_URL;
    @Override
    ...
    public ResultData call( IdPo req, String token) {
        try{
            String resp = invoke(req,null,token, API_B2S2C_URL+API_B2S2C_ORDER_DELETE_URL);
