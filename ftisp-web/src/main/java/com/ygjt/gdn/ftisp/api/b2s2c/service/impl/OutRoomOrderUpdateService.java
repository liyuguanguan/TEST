 ...
 */
@Service
public class OutRoomOrderUpdateService extends AbstractB2S2CInfService<OutroomOrderUpdatePO,ResultData> implements InterService<OutroomOrderUpdatePO,ResultData> {
    private static final Logger logger = Logger.getLogger(OutRoomOrderUpdateService.class);

    @Value("#{configProperties['api.b2s2c.outRoomOrder.update.url']}")
    protected String            API_B2S2C_OUTROOMORDER_UPDATE_URL;
    @Override
    public ResultData call(OutroomOrderUpdatePO req) {
        return null;
    }
    @Override
    public ResultData call( OutroomOrderUpdatePO req, String token) {
        try{
            String resp = invoke(null,req,token, API_B2S2C_URL+API_B2S2C_OUTROOMORDER_UPDATE_URL);
