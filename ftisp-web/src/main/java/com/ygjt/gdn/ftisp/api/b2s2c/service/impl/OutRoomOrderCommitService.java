 ...
 */
@Service
public class OutRoomOrderCommitService extends AbstractB2S2CInfService<List<OutroomOrderIdPO>,ResultData<?>> implements InterService<List<OutroomOrderIdPO>,ResultData<?>> {
    private static final Logger logger = Logger.getLogger(OutRoomOrderCommitService.class);

    @Value("#{configProperties['api.b2s2c.outRoomOrder.commit.url']}")
    protected String            API_B2S2C_OUTROOMORDER_COMMIT_URL;
    @Override
    ...
    public ResultData<?> call( List<OutroomOrderIdPO> req, String token) {
        try{
            String resp = invoke(null,req,token, API_B2S2C_URL+API_B2S2C_OUTROOMORDER_COMMIT_URL);
