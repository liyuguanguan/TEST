 ...
 */
@Service
public class GoodsStockQueryGoodsService extends AbstractB2S2CInfService<GoodsStockQueryGoodsPO,ResultData<List<GoodsStockQueryGoodsVO>>> implements InterService<GoodsStockQueryGoodsPO,ResultData<List<GoodsStockQueryGoodsVO>>> {
    private static final Logger logger = Logger.getLogger(GoodsStockQueryGoodsService.class);

    @Value("#{configProperties['api.b2s2c.goodsStock.queryGoods.url']}")
    protected String            API_B2S2C_GOODSSTOCK_QUERYGOODS_URL;
    @Override
    ...
    public ResultData<List<GoodsStockQueryGoodsVO>> call( GoodsStockQueryGoodsPO req, String token) {
        try{
            String resp = invoke(req,null,token, API_B2S2C_URL+API_B2S2C_GOODSSTOCK_QUERYGOODS_URL);
