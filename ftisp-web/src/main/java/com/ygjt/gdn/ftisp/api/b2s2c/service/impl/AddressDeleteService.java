 ...
 */
@Service
public class AddressDeleteService extends AbstractB2S2CInfService<List<IdPo>,Result> implements InterService<List<IdPo>,Result> {
    private static final Logger logger = Logger.getLogger(AddressDeleteService.class);

    @Value("#{configProperties['api.b2s2c.address.delete.url']}")
    protected String            API_B2S2C_ADDRESS_DELETE_URL;
    @Override
    ...
    public Result call(List<IdPo> req, String token) {
        try{
            String resp = invoke(null,req,token, API_B2S2C_URL+API_B2S2C_ADDRESS_DELETE_URL);
