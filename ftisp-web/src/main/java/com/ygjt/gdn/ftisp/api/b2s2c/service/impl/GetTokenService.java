 ...
 */
@Service
public class GetTokenService extends AbstractB2S2CInfService<GetTokenReq,GetTokenResp> implements InterService<GetTokenReq,GetTokenResp> {
    private static final Logger logger = Logger.getLogger(GetTokenService.class);
    ...
    @Value("#{configProperties['api.b2s2c.grant.type']}")
    private String            API_B2S2C_GRANT_TYPE;
    @Override
    public GetTokenResp call(GetTokenReq req) {
        try{
            req.setClient_id(API_B2S2C_CLIENT_ID);
            req.setClient_secret(API_B2S2C_CLIENT_SECRET);
            req.setGrant_type(API_B2S2C_GRANT_TYPE);
    ...
    @Override
    public GetTokenResp call(GetTokenReq param,  String token) {
        return null;
    }
    @Override
